...
try:
    ...
    # ----------------------------------
    # Form: all controls & Run button
    # ----------------------------------
    with st.form("cluster_form", clear_on_submit=False):
        ...
        if len(numerical_columns) == 0 and len(categorical_columns) == 0 and len(text_columns) == 0:
            st.warning("Select at least one numerical, categorical, or text column.")
        ...
        with st.expander("Text & Dimensionality Settings", expanded=True):
            ...
            default_index = 1 if (_HAS_ST) else 0
            text_method = st.radio(
                "Text embedding method",
                text_method_options,
                index=default_index if len(text_method_options) > 1 else 0,
                help=("Install sentence-transformers to enable embeddings." if not _HAS_ST else None),
            )

            # Only show TF-IDF specific controls if TF-IDF is chosen AND text columns are selected
            tfidf_ngrams = (1, 2)
            max_features = 20000
            if (text_method == "TF-IDF") and (len(text_columns) > 0):
            ...
            text_reducer_type = st.radio(
                ...
                ["UMAP", "SVD"],
                index=0,
                help="SVD: fast for large sparse TF-IDF. UMAP (cosine): good for embeddings/TF-IDF.",
            )
    ...
    # ----------------------------------
    # Execute only when "Run clustering" is pressed
    # ----------------------------------
    if submitted:
        if len(numerical_columns) == 0 and len(categorical_columns) == 0 and len(text_columns) == 0:
            st.error("Please select at least one numerical, categorical, or text column before running.")
            st.stop()

        # Build features
        X_num, _ = scale_numeric(data, numerical_columns)

        X_cat, _ = one_hot_categorical(data, categorical_columns)
        ...
        # Visualization (UMAP on feature space, parameters come from the form)
...
2. Use the form to select numerical, categorical, and/or text columns and configure options.
3. Click **Run clustering** to execute.
4. Inspect the clustered data and the UMAP plot. Download results if needed.
"""
)
